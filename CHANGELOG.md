...
- Documented ForgeLoop-owned deterministic Flutter guide selection. Bridge
  transports canonical ordered `selectedGuideIds` values, including unknown
  additive IDs, without detecting projects, selecting guides, or treating a
  selected guide as evidence, authority, verification, or completion.

### Fixed

- Validate canonical `selectedGuideIds` as a bounded list of non-empty strings
  before exposing it to Worker coordination code. No ForgeLoop or Flutter
